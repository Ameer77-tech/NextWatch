...
const Container = () => {
  const Popular = useHomeData((state) => state.Popular);
  ...
  useEffect(() => {
    if (cardRef.current && containerRef.current) {
      ...
      const totalCardsW = cardW * Popular.length;
      const maxDragDistance = totalCardsW - containerW;
      setMaxDrag(maxDragDistance > 0 ? maxDragDistance : 0);
    }
  }, [Popular, cardRef, containerRef]);

  const handleNext = () => {
    setcurrentIndex((prev) =>
      prev !== Math.floor(Popular.length / 5) - 1 ? prev + 1 : (prev = prev)
    );
  };
  const handlePrev = () => {
